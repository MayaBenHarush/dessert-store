...
// גישה למנהל בלבד (משתמש 'admin' לפי העוגייה session).
...
module.exports = (app) => {
  ...
  // עזר ליצירת מזהה (slug) ייחודי למוצר
  ...
  // מחזיר לוג פעילות (login/logout/add-to-cart/…); סינון לפי תחילת שם משתמש.
  ...
  // הערה: image יכול להיות שם קובץ מתיקיית public/images או URL מלא.
  app.post('/api/admin/products', async (req, res, next) => {
    try {
      ...
      const product = {
        ...
        image: String(image) // לדוגמה: "chocolate-cake.jpg" או "https://…"
      };
  ...
  // בנוסף מנקה אותו מכל הסלים הקיימים.
